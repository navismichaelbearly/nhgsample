...
describe('TaskCloseModal connector', () => {
  let dispatchProps;
  let mockState;
  ...
  describe('mapStateToProps', () => {
    ...
    describe('mergeProps', () => {
      ...
      it('performs the correct action when onCancel fires', () => {
        result.onCancel();
      ...
      it('performs the correct actions for onSubmit', () => {
        ...
        expect(taskActions.closeTask).toHaveBeenCalledWith(ownProps.task.id, {
          ...payload,
          status: 'Completed',
        });
        expect(modalActions.setModalContent).toHaveBeenCalled();
